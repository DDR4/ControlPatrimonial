...
namespace CP.DataAccess
{
    public class DAUsuario
    {
        ...
        public Usuario GetUsuarioDni(Usuario obj)
        {
            using (var connection = Factory.ConnectionFactory())
            {
                connection.Open();
                ...
                parm.Add("@Dni", obj.Dni);
                var result = connection.Query(
                     sql: "sp_Buscar_Usuario_Dni",
                     param: parm,
                     commandType: CommandType.StoredProcedure)
                     .Select(m => m as IDictionary<string, object>)
                     .Select(n => new Usuario
                     {
                         Usuario_Id = n.Single(d => d.Key.Equals("TablaReniec_Id")).Value.Parse<int>(),
                         Nombres = n.Single(d => d.Key.Equals("TablaReniec_Nombres")).Value.Parse<string>(),
                         Apellidos = n.Single(d => d.Key.Equals("TablaReniec_Apellidos")).Value.Parse<string>(),
                         Dni = n.Single(d => d.Key.Equals("TablaReniec_Dni")).Value.Parse<string>(),
                     });
